...
export const getAllCandidates = async (params) => {
    try {
        ...
        if (params?.search) {
            query = query.filter(candidate => candidate.name.toLowerCase().includes(params.search.toLowerCase()) ||
...
export const getCandidateStatistics = async () => {
    ...
    const newCandidates = allCandidates.filter(candidate => new Date(candidate.appliedAt) >= oneWeekAgo);

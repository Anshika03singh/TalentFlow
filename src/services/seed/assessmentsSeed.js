...
function generateQuestion(sectionIndex, questionIndex) {
    ...
    if (type === 'single-choice' || type === 'multi-choice') {
        baseQuestion.options = Array.from({ length: faker.number.int({ min: 3, max: 5 }) }, () => faker.lorem.words(2));
    }
...
function generateAssessment(jobId) {
    ...
    return {
        ...
        sections: Array.from({ length: sectionsCount }, (_, sectionIndex) => ({
            ...
            questions: Array.from({ length: faker.number.int({ min: 3, max: 6 }) }, (_, questionIndex) => generateQuestion(sectionIndex, questionIndex))
        })),

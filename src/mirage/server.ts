  ...
  const server = createServer({
      ...
      job: Factory.extend({
        ...
        tags() {
          return faker.helpers.arrayElements(
            ["Engineering", "Marketing", "Sales", "Design", "HR"],
            2
          );
        },
        order(i: number) {
    ...
    routes() {
      ...
      const simulateDelay = async () =>
        new Promise((r) =>
          setTimeout(r, faker.number.int({ min: 200, max: 1200 }))
        );

      // --- JOB ROUTES ---
      this.get("/jobs", async (schema: any, request: any) => {
        await simulateDelay();
        let jobs = schema.all("job").models;
        const { search = "", status = "", page = 1, pageSize = 10 } =
          request.queryParams;
      ...
      this.post("/jobs", async (schema: any, request: any) => {
        await simulateDelay();
        if (Math.random() < 0.1)
      ...
      this.patch("/jobs/:id", async (schema: any, request: any) => {
        await simulateDelay();
      ...
      this.patch("/jobs/:id/reorder", async (schema: any, request: any) => {
        await simulateDelay();
        if (Math.random() < 0.1)
          return new Response(500, {}, { error: "Reorder failed" });
        const { fromOrder, toOrder } = JSON.parse(request.requestBody);
        const jobs = schema.all("job").models;
        const job = jobs.find((j: any) => j.order === fromOrder);
        if (job) job.update({ order: toOrder });
        return { success: true };
      });

      // --- CANDIDATE ROUTES ---
      this.get("/candidates", async (schema: any, request: any) => {
        await simulateDelay();
        let candidates = schema.all("candidate").models;
        const { search = "", stage = "", page = 1, pageSize = 20 } =
          request.queryParams;

        if (search) {
          candidates = candidates.filter((c: any) =>
            [c.name, c.email].some((val) =>
      ...
      this.patch("/candidates/:id", async (schema: any, request: any) => {
        await simulateDelay();
        const id = request.params.id;
        const attrs = JSON.parse(request.requestBody);
      ...
      this.get("/candidates/:id/timeline", async (_schema: any, request: any) => {
        await simulateDelay();
        const id = request.params.id;
        return {
          id,
      ...
      // --- ASSESSMENT ROUTES ---
      this.get("/assessments/:jobId", async (schema: any, request: any) => {
        await simulateDelay();
        const jobId = Number(request.params.jobId);
        return schema.findBy("assessment", { jobId });
      });

      this.put("/assessments/:jobId", async (schema: any, request: any) => {
        await simulateDelay();
        const jobId = Number(request.params.jobId);
        const attrs = JSON.parse(request.requestBody);
      ...
      this.post("/assessments/:jobId/submit", async (_schema: any, request: any) => {
        await simulateDelay();
        const data = JSON.parse(request.requestBody);
        localStorage.setItem(
          `assessment-response-${data.jobId}-${data.candidateId}`,

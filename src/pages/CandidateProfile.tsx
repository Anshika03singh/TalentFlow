  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8 flex justify-between items-start">
        ...
        <button
          onClick={() => navigate("/dashboard/candidates")}
          className="px-4 py-2 text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-emerald-500"
        >
          Back
        </button>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <span className={`px-3 py-1 text-sm font-medium rounded-full ${currentStageInfo.color}`}>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              <p><span className="font-medium text-gray-500">Applied Date:</span> {new Date(candidate.appliedAt).toLocaleDateString()}</p>
              <p><span className="font-medium text-gray-500">Last Updated:</span> {new Date(candidate.updatedAt).toLocaleDateString()}</p>
              {candidate.phone && <p><span className="font-medium text-gray-500">Phone:</span> {candidate.phone}</p>}
              {candidate.experience && <p><span className="font-medium text-gray-500">Experience:</span> {candidate.experience}</p>}
            </div>
          </div>
            ...
            {notes && (
              <div className="mt-4">
                ...
                <div className="bg-gray-50 p-3 rounded-md">
                  <pre className="text-sm text-gray-900 whitespace-pre-wrap">{notes}</pre>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mt-6">
            ...
            {loading ? (
              ...
              <ul className="space-y-3">
                {job.assessments.map((assessment: any) => (
                  <li key={assessment.id} className="p-4 border rounded-lg shadow-sm hover:shadow-md transition">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium">{assessment.title}</p>
                        <p className="text-sm text-gray-500">
                          Score: <strong>{assessment.score ?? "Not evaluated"}</strong>
                        </p>
                      </div>
                      <button
                        className="text-blue-600 hover:underline"
                        onClick={() =>
                          window.location.assign(`/assessment-results/${assessment.id}`)
                        }
                      >
                        View Results
                      </button>
